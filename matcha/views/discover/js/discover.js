...
function filter(code_html){
  ...
  for(var i= 0; i < code_html.length - 1; i++)
  {
    ...
    //  code_html[i].ponderation ==
      if( code_html[i].ponderation == table2.length && code_html[i].email != code_html[code_html.length - 1].mine && code_html[i].distance < location && code_html[i].age > age_from && code_html[i].age < age_to && code_html[k].block.indexOf(code_html[i].email) == -1 && code_html[i].popularity > popularity_from && code_html[i].popularity < popularity_to)
      {
