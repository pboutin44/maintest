...
function loadStreamPage (url, movie) {
	...
	$.ajax({
		...
		success  : function(resp) {
			if (resp.success) {
				var query = jQuery.param(movie);
				var redirect_uri = resp.redirect + '?' + query;
...
function sortAndDisplayArchive(movies, search) {
  $.ajax({
    method  : "GET",
    url     : "/home/research/movie_already_seen",
    error    : function(request, error) {
      console.log("ERROR : " + error + " RESP : " + request.responseText);
    },
        ...
        if (display) {
					...
					'<div class="movie_box">' +
						'<p class="movie_title">'
							...
							+ movie.title + '</p>' +
						'<div>' +
								'<img class="movie_img img-rounded img-responsive" src="https://archive.org/services/img/'+movie.identifier+'" alt="" title="" width=400 length=300 />' +
								'</a><p>IMDB:' + (movie.avg_rating * 2) + ' YEAR: '+ date + '</p>' +
						'</div>' +
					'</div>';
...
function register(identifier){
  $.ajax({
    method  : "get",
    url     : "/home/research/addviewmovie",
...
function sortAndDisplayLegitT(movies, search) {
  ...
  if (search.sort == 3) { // (Sort by rating)
  ...
  movies.forEach(function (movie) {
    ...
    if (movie) {
      ...
      if (display) {
        console.log("DISPLAY : " + JSON.stringify(movie));
        $("#movies").append('<div class="movie_container col-lg-3 col-sm-4 col-xs-6">' +
                                '<div class="movie_box">' +
                                  '<p class="movie_title">' + movie.title + '</p>' +
                                  '<a href="http://localhost:8080/torrent/download/' + encodeURIComponent(movie.torrent) + '">' +
                                    '<img class="movie_img img-rounded img-responsive" src="' + movie.poster + '" alt="" title=""/>' +
                                  '</a><p> IMDB: ' + movie.rating + ' YEAR: ' + movie.year + '</p>' +
                                '</div>' +
                              '</div>');
...
function scrapLegitTorrents(search, cb) {
  ...
  $.ajax({
    method  : "GET",
    url     : "/home/research/legittorrenttest",
    data : {search  : search,
            page    : g_page},
    error    : function(request, error) {
      console.log("ERROR : " + error + " RESP : " + request.responseText);
