...
module.exports = function(Peer) {
	Peer.prototype.piece = function(msgBuffer) {
		...
		logger.debug('blockQueue', this.currentPiece.blockQueue, 'msgBufferHead', msgBufferHeader);
		...
		if (headerIndex >= 0) {
			...
			// this.currentPiece.blockQueue.shift();
			// this.currentPiece.data = Buffer.concat([this.currentPiece.data, piece.dataBlock]);
			this.currentPiece.blockQueue[headerIndex].data = piece.dataBlock;
			var i = 0;
			while (i < this.currentPiece.blockQueue.length && this.currentPiece.blockQueue[i].data != null) {
				this.currentPiece.data = Buffer.concat([this.currentPiece.data, this.currentPiece.blockQueue[i].data]);
				this.currentPiece.blockQueue.splice(i, 1);
				i++;
			}
		...
		if (this.watchDlSpeed == false) {
			if (this.bt.peersAlive.length < this.bt.MAX_ALIVE_PEERS) {
				this.bt.peersAlive.push({
			...
			this.watchDlSpeed = setInterval(() => {
				...
				if (this.downloadSpeed < 42 && this.amChoke == false) {
					// streaming case
	...
	Peer.prototype.request = function() {
		...
		logger.silly('currentPieceOffset before pipelining req: ' + this.currentPiece.offset +
		...
		for (var i = 0, offset = this.currentPiece.offset; (i < 5) &&
			...
			this.currentPiece.blockQueue[this.currentPiece.blockQueue.length - 1].data = null;
		...
		logger.silly('Full request message: ', requestPiecesBuf.toString('hex'));
	...
	Peer.prototype.chosePiece = function() {
		...
		if (piece == -1) {
			// Do not kill if fast peer nt reuse it (in case of small torrent file)
	...
	Peer.prototype.pieceFinished = function() {
		if (require('crypto').createHash('sha1').update(this.currentPiece.data).digest('hex') ==
			...
			for (var i = 0; i < this.bt.peersAlive.length; i++) {
				if (this.bt.peersAlive[i].href == this.remoteClientHref)
					this.bt.peersAlive[i].nbPiecesDl++;
			}
	...
	Peer.prototype.writeToFiles = function() {
		for (var i = 0; i < this.bt.files.length; i++) {
			...
			if (pieceIndex >= 0) {
				// logger.debug(pieceIndex, this.bt.files[i]);
				...
				if (alignPiecesNb > 0) {
					...
					((i) => {
						...
						// will be more precise if make in queue
				...
				// /!\ ? nasa pictures files case
				logger.silly('Cannot find piece nb ' + this.currentPiece.index);
				// /!\ ?
			}
		}
	}
