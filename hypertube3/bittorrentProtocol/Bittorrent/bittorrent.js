...
// var streamEventEmitter = new StreamEventEmitter();
...
Bittorrent.prototype.start = function() {
	this.peerId = '-HT0101-' + crypto.randomBytes(6).toString('hex');
	this.metadata.announce.forEach((announceUrl, i) => {
		var parsedAnnounceUrl = url.parse(announceUrl);
		...
		if (parsedAnnounceUrl.protocol == 'http:') {
			...
			this.httpTracker[i].firstCall = true;
			this.reqHttpTracker(i);
		}
			...
			this.udpTracker[i].firstCall = true;
			this.reqUdpTracker(i);
		}
	});
}
...
/* don't work, nt be same struc as currentLen
Bittorrent.prototype.percent = function(filesPath, cb) {
	if (typeof filesPath === 'object') {
		this.files.forEach((file) => {
			if (file.started == true) {
				...
				if (file.currentLen == file.length)
					...
					percent = Math.round((file.currentLen * 100 / parseInt(file.length)) * 10) / 10;
					// logger.verbose('[' + file.name + '] : ' + percent);
				}
				cb(percent); // ^ put in else if no need to put info when finished
			}
		});
	}
}
